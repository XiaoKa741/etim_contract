...
contract ETIMMain is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public participationAmountMax; // 150 USD (6 decimals), set in initialize()
    ...
    // Team depth
    uint256 public maxTeamDepth;  // max recursion depth for team balance propagation, set in initialize()
    ...
    uint256 public lpBurnCooldown;     // set in initialize()
    uint256 public lpBurnLastTrigger;
    uint256 public lpBurnAutoRatio;    // ratio applied to LP and burn portions separately, set in initialize()

    // LP+Burn manual trigger
    uint256 public lpBurnManualRatio;    // manual allocation ratio, set in initialize()
    ...
    uint256 public ethPriceInUsd;   // 1 ETH = 2000 USD (6 decimals), set in initialize()
    uint256 public ethPriceInEtim;  // 1 ETH = 2000 ETIM (18 decimals), set in initialize()
    uint256 public etimPerUsd;      // 1 USD = 1 ETIM (18 decimals), set in initialize()
    ...
    uint256 public dailyDepositRate;   // 20% = 200/1000, set in initialize()
    ...
    // Whitelist
    mapping(address => bool) public tokenCallbackWhitelist;

    // Withdraw target addresses (set by owner, anyone can trigger withdraw)
    address public foundationWithdrawAddr;
    address public potWithdrawAddr;
    address public officialWithdrawAddr;

    // MODIFIERS
    modifier onlyEtimTokenOrOwner() {
        if (msg.sender != address(etimToken) && msg.sender != owner() && !tokenCallbackWhitelist[msg.sender]) revert OnlyEtimToken();
        _;
    }
    ...
    function _initializeLevelConditions() internal {
        ...
        uint256[7] memory minD = [uint256(0),5,10,15,20,25,30];
        uint256[7] memory minP = [uint256(0),10000,20000,30000,40000,50000,60000];
        uint256[7] memory minT = [uint256(0),100000,200000,400000,500000,600000,700000];
        uint256[7] memory accR = [uint256(10),7,10,12,15,18,21];
        
        for (uint8 i = 0; i < 7; i++) {
            levelConditions[i] = LevelCondition(
                minD[i],
                minP[i] * 10**18,
                minT[i] * 10**18,
                accR[i]
    ...
    function _processParticipation(address addr, uint256 ethAmount) private {
        ...
        // participants.push(addr); // Deprecated: array never read
        totalUsers++;
    ...
    function _processReferralBinding(address from, address to, uint256 value) private {
        if (
            ...
            if (forwardTime > 0 && reverseTime > 0) {
                ...
                delete transferRecords[referrer][invitee];
                delete transferRecords[invitee][referrer];
            }
        }
    }
    ...
    function getSmallZoneTokens(address user) external view returns (uint256) {
    ...
    function setMyInvitee(address[] calldata inviteeList) external {
        if (users[msg.sender].participationTime == 0) revert NotParticipated();
        for (uint256 i = 0; i < inviteeList.length; i++) {
            _setReferrer(inviteeList[i], msg.sender);
        }
    }
    ...
    function _setReferrer(address invitee, address referrer) private {
        if (invitee == address(0) || referrer == address(0)) revert ZeroAddress();
        if (invitee == referrer || referrerOf[invitee] != address(0)) revert InvalidParams(); // already has referrer
    ...
    function setTokenCallbackWhitelist(address addr, bool allowed) external onlyOwner {
        tokenCallbackWhitelist[addr] = allowed;
    }

    // Set withdraw target addresses
    function setWithdrawTargets(address _foundation, address _pot, address _official) external onlyOwner {
        foundationWithdrawAddr = _foundation;
        potWithdrawAddr = _pot;
        officialWithdrawAddr = _official;
    }

    // withdraw foundation (WETH)
    function withdrawFoundation() external nonReentrant {
        address to = foundationWithdrawAddr;
    ...
    // withdraw pot (WETH)
    function withdrawPot() external nonReentrant {
        address to = potWithdrawAddr;
    ...
    function withdrawOfficial() external nonReentrant {
        address to = officialWithdrawAddr;
